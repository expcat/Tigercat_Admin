...
- **现状**：双端各 22 个功能页（React 19 / Vue 3 等价实现，含 `/monitor`），组件覆盖接近完成——组件库中明确未演示的仅剩 `VirtualList`、`VirtualTable`、`Drag`、`Kanban` 4 个（见文末勾选表）。
...
| 列表→详情模板页（动态路由详情） | ✓ | – | – | – | ✓ | 22 页均为功能页，无 `/:id` 详情 | 阶段 9 |
...
| 9 | 监控页与列表-详情模板 | `/monitor`、`/projects*` | 进行中（`/monitor` 已完成） |
