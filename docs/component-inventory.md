...
本轮复核未发现需要新增到 [upstream-requirements.md](upstream-requirements.md) 的 Tigercat 组件 API 或可访问性缺口。Vue 侧列开关在移动端以内联 checkbox 方式呈现，React 侧通过 Popover 呈现，测试按“列开关可达”语义兼容两端差异。
