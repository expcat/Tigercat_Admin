...
| Home | `pages/HomePage.tsx` / `pages/HomePage.vue` | `Alert`、`Card`、`Text`、`Tag`、`Select`、`LineChart`、`BarChart`、`PieChart`、`Loading`、`Statistic`、`Empty` | 统计块、系统信息和图表空状态已收敛到共享片段；欢迎区和图表数据色仍有页面级布局；快捷操作已在双端通过路由实现点击跳转 | 双端页面内容和布局基本对齐；React 图标来自 `Icons.tsx`，Vue 图标来自 `Icon.vue`；Vue 侧 AppLogo 统一为 48px | 折线图采用 strokeGradient/pointGradient，柱状图/饼图采用 gradient/shadow 效果，饼图标签采用 outside 保持一致 | `pnpm build:frontend`；桌面/移动截图；首页数据加载 smoke | 暂无新增上游缺口 |
...
- P9 已完成 Tigercat UI 1.2.16 升级流程复核；本轮未发现新的上游组件 API、交互、可访问性或性能缺口。

## P9 复核记录

P9 复核时通过 npm registry 确认 `@expcat/tigercat-core`、`@expcat/tigercat-react`、`@expcat/tigercat-vue` 的 `latest` 均为 `1.2.16`，与当前 `package.json` / `pnpm-lock.yaml` 一致，因此本轮不进行依赖版本 bump。

迁移说明复核集中在 v0.5.0、v1.0.0 与 v1.2.0 的破坏性变更：弹出层统一 `open` / `update:open` / `onOpenChange`，Vue 事件命名统一 kebab-case，Button 原生按钮类型使用 `htmlType`，Image 预览可见性旧命名移除。源码关键词复核未发现旧 `visible` / `update:visible` / `onVisible*` 模式；React 继续使用 `open`、`on*Change` 与 `className`，Vue 继续使用 `v-model:open`、`@update:*` 与 `class`。

重组件导入路径保持 P3 策略：图表、`TaskBoard`、`NotificationCenter`、`FileManager`、`ActivityFeed`、`Timeline`、`ColorPicker`、`Upload`、`CropUpload` 继续使用 React / Vue 对应子路径导入，普通基础组件继续使用包级导入。`pnpm build:frontend` 的重点体积为 React `vendor-ui` 335.24 kB / gzip 100.47 kB，Vue `vendor-ui` 377.23 kB / gzip 108.64 kB。

本轮复核未发现需要新增到 [upstream-requirements.md](upstream-requirements.md) 的 Tigercat 组件 API、交互、可访问性或性能缺口；当前暂无最小复现或示例页面需要补充。
...
- 2026-06-05：`pnpm build:frontend` 通过，确认 P9 复核后的双端生产构建、重组件子路径导入和 `vendor-ui` 体积记录正常；`@expcat/tigercat-*` 三包 `latest` 与当前锁定版本均为 1.2.16，未执行依赖升级。
