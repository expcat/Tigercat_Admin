...
### Milestone F：核心 E2E 闭环

目标：把现有 Playwright 基座从认证与页面烟测扩展为真实后台主流程回归。

范围：

- 补齐 React / Vue 双端共享登录夹具、会话恢复和退出后的路由保护断言。
- 增加用户 CRUD 主流程：创建、编辑、状态切换、角色分配、单条删除和批量删除。
- 增加角色权限主流程：创建角色、编辑描述、配置权限树、保护系统角色删除规则。
- 增加系统设置主流程：保存确认、取消不落库、恢复默认值、刷新后持久化回显。
- 统一 `e2e_` 测试数据前缀、唯一后缀和清理策略，保证重复运行稳定。
- 将 `pnpm e2e:react`、`pnpm e2e:vue` 纳入本地门禁，并为后续 CI 接入预留项目配置。

验证：

- `pnpm e2e:react`
- `pnpm e2e:vue`
- 失败信息能区分前端交互、权限拦截和后端接口问题。
...
1. 扩展 Playwright 用例到用户 CRUD 主流程，先覆盖 React / Vue 双端成功路径。
2. 补角色权限配置与系统设置保存的 E2E，用固定 `e2e_` 数据约束保证可重复运行。
3. 设计媒体资源 API 与数据模型，同步更新 [api.md](api.md)。
4. 接入站点 Logo 与用户头像的真实持久化流程，替换当前本地预览占位。
5. 将任务面板从本地看板状态迁移到后端任务模型。
