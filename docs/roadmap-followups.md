...
## 阶段 8 — Shell 进阶（多标签、锁屏已完成；水印 / 主题抽屉未做）
...
- [ ] **键盘路径**：标签条 Tab / Enter 切换；关闭按钮可达；「标签操作」下拉 Esc 关闭并恢复焦点。锁屏 Esc / ⌘K 不可绕过 PIN；解锁后焦点回到 Shell（demo e2e 已覆盖锁定、错误 PIN、正确 PIN 与路由保持，焦点恢复尚未断言）。

### 说明

- 多标签与锁屏已落地。全局水印开关、主题配置抽屉仍待后续步骤。
