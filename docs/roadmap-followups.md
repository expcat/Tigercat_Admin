...
## 阶段 8 — Shell 进阶（多标签已完成；锁屏 / 水印 / 主题抽屉未做）

### 人工核验（自动化 e2e 未覆盖）

- [ ] **移动端 375px**：标签条横向滚动不撑破视口；右侧「标签操作」下拉不被裁切；关闭按钮可点。
- [ ] **暗色模式（`.dark`）**：活动/非活动 `Tag` 对比可读，标签条边框走 token。
- [ ] **键盘路径**：标签条 Tab / Enter 切换；关闭按钮可达；「标签操作」下拉 Esc 关闭并恢复焦点。

### 说明

- 本步只落地多标签页导航。锁屏、全局水印开关、主题配置抽屉仍待后续步骤。
